...
def create_new_data_stream() -> dict:
    ...
    _data_stream['info']['streamer'] = CHAT_ROOM[0]
...
def fetch_viewers_current() -> int:
    return len(bot.viewers['in_chat'][CHAT_ROOM[0]])
            ...
            if user_input == "":
                ...
                for x in range(times_run):
                    try:
                        ...
                        logger.error(f"{fortime()}: ERROR 'on_stream_stats/auto-refresh' - {_error}")
                        try:
                            continue
